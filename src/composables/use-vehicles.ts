...
export function useVehicles() {
  const vehicleModels = ref<VehicleModel[]>([
    ...
    { id: 10, brand: 'Suzuki', model: 'Raider 150', yearRange: '2013-2020', engineCc: 150 },
    { id: 11, brand: 'Kawasaki', model: 'Ninja 250 / Z250', yearRange: '2013-2017', engineCc: 250 },
  ]);
  ...
  const addVehicleModel = (newModel: Omit<VehicleModel, 'id'>): void => {
    const id = vehicleModels.value.length > 0 ? Math.max(...vehicleModels.value.map((v) => v.id)) + 1 : 1;
